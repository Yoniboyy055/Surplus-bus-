...
export async function GET() {
  ...
  try {
    ...
    // Prefer the SQL function (024_last_run_per_parser.sql); fall back to JS grouping.
    const { data: rpcData, error: rpcErr } = await admin.rpc("last_run_per_parser");

    if (!rpcErr && rpcData) {
      parserStatus = (rpcData as { parser_key: string; last_started_at: string; last_status: string }[]);
    } else {
