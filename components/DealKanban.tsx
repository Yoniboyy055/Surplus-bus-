...
const COLUMNS = [
  { id: 'NEW_SUBMISSION', title: 'New', color: 'bg-blue-500/10 border-blue-500/20 text-blue-400' },
  ...
  { id: 'MATCHING', title: 'Matching', color: 'bg-cyan-500/10 border-cyan-500/20 text-cyan-400' },
        ...
        {COLUMNS.map(column => {
          ...
          return (
            <div key={column.id} className="min-w-[300px] max-w-[300px] flex flex-col">
              ...
              <div className="bg-slate-900/30 border-x border-b border-slate-800 rounded-b-lg flex-1 p-2 space-y-3">
                {columnDeals.map(deal => (
                  <div 
                    key={deal.id}
                    onClick={() => setSelectedDeal(deal)}
                    className="bg-slate-950 p-3 rounded border border-slate-800 hover:border-slate-600 hover:shadow-lg transition cursor-pointer group"
                  >
                    <div className="flex justify-between items-start mb-2">
                       <span className="font-mono text-[10px] text-slate-500 group-hover:text-blue-400 transition">#{deal.id.substring(0, 6)}</span>
