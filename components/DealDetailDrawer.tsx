...
export function DealDetailDrawer({ isOpen, onClose, deal, onStatusChange }: DealDetailDrawerProps) {
  ...
  const handleAction = async (status: string) => {
    ...
    setLoading(true);
    try {
      await onStatusChange(deal.id, status, clarificationMessage, internalNote);
      onClose();
    } catch (error) {
      console.error(error);
    } finally {
          ...
          <section className="bg-slate-950 rounded-lg p-5 border border-slate-800">
            ...
            <div className="space-y-4">
               ...
               <div className="grid grid-cols-2 gap-3">
                  <Button size="sm" variant="secondary" onClick={() => handleAction('QUALIFIED')} disabled={loading}>
                    Qualify Deal
                  </Button>
                  <Button size="sm" variant="secondary" onClick={() => handleAction('NEEDS_CLARIFICATION')} disabled={loading}>
                    Request Info
                  </Button>
                  <Button size="sm" variant="secondary" onClick={() => handleAction('REJECTED')} disabled={loading}>
                    Reject
                  </Button>
                  <Button size="sm" variant="primary" onClick={() => handleAction('MATCHING')} disabled={loading}>
                    Start Matching
                  </Button>
               </div>
